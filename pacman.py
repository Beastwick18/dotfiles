...
def get_info(package):
    ...
    for line_num in range(len(content) - 1):
        if "Optional Deps" not in content[line_num]:
            ...
            end_line = [i for i in range(len(content)) if "Required By" in content[i]][
                0
            ]
            ...
            try:
                if ":" in content[line_num]:
                    ...
                    opt_dep[content[line_num][1].strip()] = content[line_num][2].strip()
            except:
                pass
...
def is_aur(package):
    ...
    try:
        ...
        matched_packages = (
            pacman("-Ssq", package, pacman_bin="pacman").get("stdout").split("\n")
        )
